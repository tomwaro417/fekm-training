...
import { Upload, Camera, FileVideo, AlertCircle, CheckCircle2, Play } from 'lucide-react'
...
export function VideoUploader({
  ...
  // Détection de la caméra disponible
  useEffect(() => {
    ...
    const checkCamera = async () => {
      try {
        ...
        // enumerateDevices peut nécessiter une permission préalable ;
        // en cas d'erreur on considère qu'aucune caméra n'est disponible.
        const devices = await navigator.mediaDevices.enumerateDevices()
        const videoDevices = devices.filter(device => device.kind === 'videoinput')
        setHasCamera(videoDevices.length > 0)
      } catch (error) {
        console.warn('Impossible de détecter la caméra:', error)
        setHasCamera(false)
      } finally {
          ...
          <div className="flex flex-col items-center text-center">
            ...
            {state.status === 'idle' && (
              <div className="flex flex-col gap-3 mt-6 w-full max-w-xs">
                {/* Choisir une vidéo depuis l'appareil */}
                <button
                ...
                {/* Filmer avec l'appareil si une caméra est disponible */}
                {cameraChecked && hasCamera && (
                  <>
                    <input
                      ...
                      onChange={handleFileSelect}
                      className="hidden"
                      id={`camera-input-${techniqueId}-${videoType}`}
                    />
                    <label
                      htmlFor={`camera-input-${techniqueId}-${videoType}`}
